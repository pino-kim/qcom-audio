...
#define AMRWBTEST_IBUFSZ (32*1024)
#define AMRWBTEST_NUM_IBUF 2
#define AMRWBTEST_IPMEM_SZ (AMRWBTEST_IBUFSZ * AMRWBTEST_NUM_IBUF)

#define AMRWBTEST_OBUFSZ (32*1024)
#define AMRWBTEST_NUM_OBUF 2
#define AMRWBTEST_OPMEM_SZ (AMRWBTEST_OBUFSZ * AMRWBTEST_NUM_OBUF)
...
struct msm_audio_aio_buf aio_ip_buf[AMRWBTEST_NUM_IBUF];
struct msm_audio_aio_buf aio_op_buf[AMRWBTEST_NUM_OBUF];
...
static void *amrwb_read_thread_8660(void *arg)
{
	struct audio_pvt_data *audio_data = (struct audio_pvt_data *) arg;
	int afd = audio_data->afd;
	int total_len;
...
static void *amrwb_write_thread_8660(void *arg)
{
	struct msm_audio_aio_buf aio_buf;
	struct audio_pvt_data *audio_data = (struct audio_pvt_data *) arg;
...
static void *amrwb_dec_event_8660(void *arg)
{
        struct audio_pvt_data *audio_data = (struct audio_pvt_data *) arg;
	...
	if (audio_data->mode) // Non Tunnel mode
		pthread_create(&evt_read_thread, NULL, amrwb_read_thread_8660, (void *) audio_data);
	pthread_create(&evt_write_thread, NULL, amrwb_write_thread_8660, (void *) audio_data);
...
static int initiate_play_8660(struct audtest_config *clnt_config)
{
	...
	int dec_id;
#endif
	int afd, ipmem_fd[AMRWBTEST_NUM_IBUF], opmem_fd[AMRWBTEST_NUM_OBUF];
	void *ipmem_ptr[AMRWBTEST_NUM_IBUF], *opmem_ptr[AMRWBTEST_NUM_OBUF];
	...
	unsigned int open_flags;
        ...
        audio_data->freq = 16000;
        audio_data->channels = 1;
        audio_data->bitspersample = 16;
	memset(ipmem_fd, 0, (sizeof(int) * AMRWBTEST_NUM_IBUF));
	memset(opmem_fd, 0, (sizeof(int) * AMRWBTEST_NUM_OBUF));
	memset(ipmem_ptr, 0, (sizeof(void *) * AMRWBTEST_NUM_IBUF));
	memset(opmem_ptr, 0, (sizeof(void *) * AMRWBTEST_NUM_OBUF));

	if(((in_size + sizeof(struct meta_in_q6)) > AMRWBTEST_IBUFSZ) ||
		(out_size > AMRWBTEST_OBUFSZ)) {
	...
	afd = open("/dev/msm_amrwb", open_flags);
	...
	if ((ioctl(afd, AUDIO_START, 0))< 0 ) {
		printf("amrwbtest: unable to start driver\n");
		goto err_state2;
	}
	if (audio_data->mode) {
		/* non - tunnel portion */
		...
		for (n = 0; n < AMRWBTEST_NUM_OBUF; n++) {
			opmem_fd[n] = open("/dev/pmem_audio", O_RDWR);
			printf("%s: opmem_fd %x\n",  __func__, opmem_fd[n]);
			opmem_ptr[n] = mmap(0, AMRWBTEST_OBUFSZ,
		...
		for (n = 0; n < (AMRWBTEST_NUM_OBUF-1); n++) {
		...
		out_free_indx = AMRWBTEST_NUM_OBUF-1;
	}
	//Register Write  buffer
	for (n = 0; n < AMRWBTEST_NUM_IBUF; n++) {
		ipmem_fd[n] = open("/dev/pmem_audio", O_RDWR);
		printf("%s: ipmem_fd %x\n",  __func__, ipmem_fd[n]);
		ipmem_ptr[n] = mmap(0, AMRWBTEST_IBUFSZ,
	...
	//Indicate available free buffer as (n-1)
	in_free_indx = AMRWBTEST_NUM_IBUF-1;
	pthread_create(&evt_thread, NULL, amrwb_dec_event_8660, (void *) audio_data);
...
err_state2:
	if (audio_data->mode) {
		for (n = 0; n < AMRWBTEST_NUM_OBUF; n++) {
			munmap(opmem_ptr[n], AMRWBTEST_OBUFSZ);
			close(opmem_fd[n]);
		}
	}
	for (n = 0; n < AMRWBTEST_NUM_IBUF; n++) {
		munmap(ipmem_ptr[n], AMRWBTEST_IBUFSZ);
		close(ipmem_fd[n]);
	}
	if (!audio_data->mode) {
#if defined(TARGET_USES_QCOM_MM_AUDIO) && defined(AUDIOV2)
